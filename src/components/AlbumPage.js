// -----------------------------------------------------------------------------
// src/components/AlbumPage.js (Mobil Uyumlu Liste)
// -----------------------------------------------------------------------------
import { useState } from 'react';

function AlbumPage({ album, onPlaySong, currentSong, onAddToQueue }) {
  ...
  return (
    <div onClick={() => setOpenMenuId(null)}>
      ...
      <div className="text-spotify-lightgray px-4 md:px-0">
        {/* Masaüstü için liste başlığı */}
        <div className="hidden md:grid grid-cols-[auto_1fr_1fr_auto_auto] gap-4 px-4 py-2 border-b border-spotify-gray text-sm">
        ...
        {album.songs && album.songs.map((song, index) => (
          <div key={song.id} className="hover:bg-spotify-gray/50 rounded-md flex items-center p-2 group">
            <div className="flex-1 flex items-center gap-4 cursor-pointer" onClick={() => onPlaySong(song, album)}>
                <span className="text-center w-5 text-spotify-lightgray hidden md:block">{index + 1}</span>
                ...
                <div className="flex-1 truncate">
                    ...
                    <p className="text-sm text-spotify-lightgray hidden md:block">{album.artist}</p>
                </div>
            </div>
            <p className="text-sm text-spotify-lightgray hidden md:block">{song.duration}</p>
